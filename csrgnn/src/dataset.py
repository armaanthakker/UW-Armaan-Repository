...
class MultiSessionsGraph(InMemoryDataset):
    ...
    def _process(self):
        ...
        if self.phrase=='train': # HACK 不同机器PYG版本不同，发布时删除这行
    ...
    def process(self):
        data_list = load_sequences_and_gen_graphs(self.raw_dir + '/' + self.raw_file_names[0])

        data, slices = self.collate(data_list)
        torch.save((data, slices), self.processed_paths[0])


def load_sequences_and_gen_graphs(sequence_fn: str):
    # print(sequence_fn)
    data = pickle.load(open(sequence_fn, 'rb'))
        
    data_list = []
    # for user, sequence, cue_l, y_l in (zip(tqdm(data[0]), data[1], data[2], data[3])):
    #     _data_list = generate_session_graph(user, sequence, cue_l, y_l)
    #     data_list.extend(_data_list)
    params = list(zip(data[0], data[1], data[2], data[3]))  # [(user, sequence, cue_l, y_l)]
    results = [_generate_session_graph(p) for p in tqdm(params)]
    # 不知道为什么多进程会报错
    # results = process_map(_generate_session_graph, params, max_workers=8, chunksize=8)
    # with Pool(processes=8) as pool:
    #     results = pool.map(_generate_session_graph, params, 8)
    for _data_list in results:
        data_list.extend(_data_list)
    return data_list


def _generate_session_graph(params):
    return generate_session_graph(*params)


def generate_session_graph(user, sequence, cue_l, y_l):
    data_list = []
